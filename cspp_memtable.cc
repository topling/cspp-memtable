...
#pragma pack(pop)
  ...
  void add_wal(size_t fileno, size_t bytes, const ReadonlyFileMmap* wal) {
    ...
    m_wals[i].wal = wal;
    m_wals[i].bytes = bytes;
    auto fno = uint32_t(fileno); // update last
    as_atomic(m_wals[i].fileno).store(fno, std::memory_order_release);
    m_hold_wals.emplace_back((ReadonlyFileMmap*)wal);
  }
...
public:
  ...
  Status Get(const ReadOptions& ro, const Slice& ikey, GetContext* get_context,
    ...
    if (m_memtab->m_ref_to_wal)
      return m_memtab->SST_Get<CSPPMemTab::KeyValueToLogRef>(ro, ikey, get_context);
    else
      return m_memtab->SST_Get<CSPPMemTab::Entry>(ro, ikey, get_context);
  }
